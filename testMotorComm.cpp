...
void setup()
{
  ...
  if(dxl.ping(DXL_ID_0)){
    Serial.println("id 1 good");
  }
...
int extend(int motor){
  ...
  dxl.setGoalCurrent(motor, -1 * targCurr, UNIT_PERCENT);
...
void displayData(){
  ...
  do {
    ...
    if(calMot == 1){
      u8g2.drawStr(40,offset + 80,"Left Motor");
    }
    else{
      u8g2.drawStr(40,offset + 80,"Right Motor");
    }
