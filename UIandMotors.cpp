...
void runMotors(){
  ...
  dxl.setGoalCurrent(DXL_ID, paramObj.dispVel * 5);
  dxl.setGoalPosition(DXL_ID, paramObj.dispExt * 500, UNIT_DEGREE);

  while(dxl.getPresentPosition(DXL_ID) < paramObj.dispExt * 500){
    Serial.print("moving: ");
    dxl.setGoalCurrent(DXL_ID, paramObj.dispVel * 5);
    dxl.setGoalPosition(DXL_ID, paramObj.dispExt * 500, UNIT_DEGREE);
  ...
  DEBUG_SERIAL.print("Present Current(Stopped) : ");
  DEBUG_SERIAL.println(dxl.getPresentCurrent(DXL_ID));
  

  dxl.setGoalCurrent(DXL_ID, -1 * (paramObj.dispVel * 5));
  dxl.setGoalPosition(DXL_ID, 0, UNIT_DEGREE);

  while(dxl.getPresentPosition(DXL_ID) > 8){
